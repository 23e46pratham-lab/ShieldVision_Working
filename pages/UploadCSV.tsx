...
import { uploadCSV, getLogs } from '../services/api';
...
const UploadCSV: React.FC = () => {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    setErrorMessage(null);
    console.log("%c[CSV] Uploading →", "color: dodgerblue; font-weight:bold;", file.name);

    try {
      const result = await uploadCSV(file);
      console.log("%c[CSV] Backend upload result:", "color: green; font-weight:bold;", result);

      setSuccess(true);

      // fetch updated logs (real backend only)
      await getLogs();

    } catch (err) {
      console.error("%c[CSV ERROR] Upload failed", "color: red; font-weight:bold;", err);
      setErrorMessage("CSV upload failed — backend could not process the file.");
    }
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-8">
          ...
          >
            ...
            <p className="text-slate-500 mt-2 text-center max-w-sm">
              {file ? `${(file.size / 1024).toFixed(2)} KB` : "Supported format: CSV (Max 50MB)"}
            </p>
        ...
        <div className="bg-slate-50 p-6 border-t border-slate-100">
          ...
          <p className="text-sm text-slate-500 mb-4">
            CSV must contain these exact columns for IBM Watson to process:
          </p>
